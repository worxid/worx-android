...
class RadioWithTextButtonTest {
    ...
    fun setUp() {
        ...
        radioWithTextButton =
            RadioWithTextButton(context, mockTextPaint, mockStrokePaint, mockCirclePaint)

        doNothing().`when`(mockStrokePaint).strokeWidth = anyFloat()
        doNothing().`when`(mockStrokePaint).style = Paint.Style.STROKE
    ...
    fun onDraw_drawCircle() {
        ...
        verify(mockStrokePaint, Times(1)).style = Paint.Style.STROKE
    ...
    fun onDraw_drawDrawable() {
        ...
        val mockCanvas = mock(Canvas::class.java)
        doNothing().`when`(mockCanvas)
            .drawCircle(anyFloat(), anyFloat(), anyFloat(), any(Paint::class.java))
        ...
        verify(mockDrawable, Times(1)).bounds = any(Rect::class.java)
    ...
    fun setTextSizeForWidth_doNotDraw() {
        ...
        doNothing().`when`(mockPaint).textSize = anyFloat()
        doNothing().`when`(mockPaint)
            .getTextBounds(anyString(), anyInt(), anyInt(), any(Rect::class.java))
        val desiredWidth = 10f
        val text = "test"

        mockPaint.setTextSizeForWidth(text, desiredWidth)
        verify(mockPaint).textSize = anyFloat()
