...
class ImageDataSourceImpl(private val contentResolver: ContentResolver) : ImageDataSource {
        ...
        return CallableFutureTask(Callable<List<Album>> {
            ...
            vc?.let {
                while (vc.moveToNext()) {
                    ...
                    val bucketId = vc.getInt(vc.getColumnIndex(BUCKET_ID)).toLong()
                    val bucketDisplayName =
                        vc.getString(vc.getColumnIndex(BUCKET_DISPLAY_NAME)) ?: continue
                    val bucketMimeType = vc.getString(vc.getColumnIndex(MIME_TYPE)) ?: continue
                    val imgId = vc.getInt(vc.getColumnIndex(MediaStore.MediaColumns._ID))
    ...
    override fun getAllBucketImageUri(
        ...
        return CallableFutureTask(Callable<List<Uri>> {
            ...
            val vc = if (bucketId != "0") {
                contentResolver.query(videos, null, selection, selectionArgs, sort)
            } else {
                contentResolver.query(videos, null, null, null, sort)
            }
            vc?.let {
                try {
                    if (vc.moveToFirst()) {
                        do {
                            val mimeType = vc.getString(vc.getColumnIndex(MIME_TYPE)) ?: continue
                            val folderName =
                                vc.getString(vc.getColumnIndex(BUCKET_DISPLAY_NAME)) ?: continue
    ...
    override fun getAlbumMetaData(
        ...
        return CallableFutureTask(Callable<AlbumMetaData> {
            ...
            val vc = if (bucketId != "0") {
                contentResolver.query(video, null, selection, selectionArgs, sort)
            } else {
                contentResolver.query(video, null, null, null, sort)
            }
                ...
                try {
                    if (vc.moveToFirst()) {
                        do {
                            ...
                            if (thumbnailPath == Uri.EMPTY) {
                                thumbnailPath =
                                    Uri.withAppendedPath(video, "" + imgId)
                            }
                            count++
                        } while (vc.moveToNext())
                    }
    ...
    override fun getDirectoryPath(bucketId: Long): CallableFutureTask<String> {
        return CallableFutureTask(Callable<String> {
            ...
            val video = MediaStore.Video.Media.EXTERNAL_CONTENT_URI
            val vc = if (bucketId != "0") {
                contentResolver.query(video, null, selection, selectionArgs, null)
            } else {
                contentResolver.query(video, null, null, null, null)
            }
            vc?.let {
                try {
                    if (vc.moveToFirst()) {
                        path = getPathDir(
                            vc.getString(vc.getColumnIndex(DATA)),
                            vc.getString(vc.getColumnIndex(DISPLAY_NAME))
                        )
                    }
                } finally {
                    if (!vc.isClosed) vc.close()
