...
class RadioWithTextButton @JvmOverloads constructor(
    ...
    constructor(context: Context, textPaint: Paint, strokePaint: Paint, circlePaint: Paint) : this(
        context
    ) {
    ...
    private var _centerRect: Rect? = null
        get() {
            if (field == null) {
                ...
                val width = width / 4
                field = Rect(
                    (r.exactCenterX() - width).toInt(),
                    (r.exactCenterY() - width).toInt(),
                    getWidth() - width,
                    height - width
                )
            }
